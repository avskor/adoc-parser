...
> **Свежий baseline 2026-05-30: Identical 135 / Different 209 / Errors 0** (344 файла).
> Актуальный приоритетный список и заметки — в `TODO.md`. Ключевое: п.11 и п.40 по факту
> уже решены/неверно описаны; п.13/п.16 закрыты (inline `[.role]`); п.38 частично закрыт
> (xref авто-текст: пустой `xref:f.adoc[]`/`<<id>>` резолвится, +56 файлов); доминирующий
> остаточный шум — NCR-кодировка типографики (`’`→`&#8217;`, 229 файлов).
