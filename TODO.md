...
- [ ] **Точечные** (ВЕРИФИЦ. 2026-06-17 vs asciidoctor; все расхождения вне корпуса → 344/344 держится):
  ...
  - [ ] п.20 (`[[id,reftext]]`) — ОТКРЫТ: reftext не используется как label `<<id>>` (у нас `[myid]`, asciidoctor `My Ref`)
  - [ ] п.28 (`toc::[]` macro) — ОТКРЫТ: нет обёртки `preamble`/`sectionbody`, `toctitle` без `class="title"`
