...
- [x] **F-BW. Автолинк рвался на `<`/`>` в теле URL** → URL с angle-bracket-плейсхолдерами (`http://<host>:<port>/realms/<realm>/token`)
  обрывался на первом `<`, оставляя `http://` (≤8 байт) → не линковался → весь URL оставался plain-текстом (ветка
  `fix/autolink-angle-brackets-in-url`, 2026-06-24). Запрос «начни следующую задачу». master `fe8742b` (F-BU смержен).
  **Триаж (свежий, метку прошлой сессии «архитектурный реордер macros/quotes» НЕ принимал — она ОШИБОЧНА):** adoc2docx
  (1105/681/291/195) — единый корень = rouge-подсветка синтаксиса (asciidoctor токенизирует Ruby/XML/Java в `<span>`),
  полноценный хайлайтер, не single-session → снят. wsl(95)+keycloak(52) — прошлая метка «реордер macros до quotes». 12 проб
  vs asciidoctor 2.0.23 ОПРОВЕРГЛИ метку: backtick-URL БЕЗ плейсхолдеров уже линкуется идентично (case 2 ✓); реальный корень —
  `<`/`>` в URL. **Правило (verified):** asciidoctor escape'ит `<`→`&lt;`, `>`→`&gt;` ДО `macros`-пасса, поэтому литеральные
  `<`/`>` в теле URL — это СОДЕРЖИМОЕ, а не терминаторы; bare-URL тянется до пробела / `[` / `]` (и закрытия constrained-спана).
  Единственное исключение — angle-форма `<scheme://…>`: ведущий `<` парится с ПЕРВЫМ `>`, оба съедаются (case 3). Пробы 1/4/5/7
  (`<x>`/`<x`/`x<y`/`a>b>c` в теле) — все включаются в URL; case 6 (backtick + плейсхолдеры) — линкуется внутри `<code>`.
  **Фикс (2 движка: subst-дефолт `subst/macros.rs` + legacy-оракул `inline.rs`):** убраны `<`/`>` из терминаторного набора
  скана URL; angle-форма теперь ищет закрывающий `>` ВНУТРИ просканированного тела (первый `>`), а не на позиции `url_end`.
  Зеркально в обоих движках (контракт «Mirror of legacy»). +2 parser unit-теста (`inline`:
  `test_autolink_keeps_angle_bracket_placeholders` + `test_autolink_angle_bracketed_bare_form_strips_brackets`), +1 subst
  дифференциальный/assertion-тест (`reproduces_legacy_on_angle_bracket_url_inputs`), +1 html_output integration-тест
  (`test_autolink_keeps_angle_bracket_placeholders_in_url`). Пре-существующий `angle_bracket_url_matches_asciidoctor`
  (closed/unclosed/`<url[text]>`/`<email>`) ПРОШЁЛ — фикс с ним согласован. clippy `--workspace` 0; **test --workspace 0
  упавших** (parser 651→**654**, html_output 48→**49**, html-lib 544, compat 233/233, integration 30, render-core 25, author 7,
  cli 2). **БАЙТ-НЕЙТРАЛЬНО:** база `/tmp/adoc_base` пересобрана от master `fe8742b` (md5 `faf8cb3e`); gate 344 — **0 diff**;
  **свип 860 файлов (4 корпуса) — изменился ТОЛЬКО 1 (целевой keycloak/index.adoc)**. Семантически (vs asciidoctor 2.0.23):
  **keycloak 52→0** (оба файла каталога identical); docs Identical **206→207**. Коммит `f26dcd5` на ветке; merge/push — ПО ЗАПРОСУ.
  - [x] **wsl (остаток 95) → F-BX (constrained-span autolink cap).** РЕШЕНО (ветка `fix/autolink-cap-at-monospace-span-close`,
    2026-06-24, 20-я сессия). URL `https://rubygems.org` в КОНЦЕ monospace-спана (`` `--plugin-source https://rubygems.org` ``,
    URL ПОСЛЕ ПРОБЕЛА мид-спана) съедал закрывающий backtick в `href` → спан не оборачивался в `<code>`, открывающий backtick
    утекал как литерал. **Триаж (свежий, 14 проб vs asciidoctor 2.0.23):** баг во ВСЕХ 4 constrained-маркерах (`` ` `` `*` `_`
    `#`), но ТОЛЬКО когда URL после пробела И в конце спана (case 2 URL-flush и case 6 URL+text-after — уже работали через F-BW).
    **Корень:** `autolink_url_limit` при space-boundary короткозамыкал в `Some(bytes.len())`, игнорируя охватывающий спан (F-BW
    капил лишь URL ВПРИТЫК к открывающему маркеру). **Фикс:** новый `enclosing_constrained_close` находит innermost охватывающий
    спан (скан маркеров слева от URL, min close > url_start) и капит скан там — pre-`quotes` стенд-ин для `<` от `</code>`.
    `macros` до `quotes`, закрывающий маркер ещё литерал. Унифицирован `autolink_url_limit` (flush И мид-спан через один cap).
    +1 subst parity-тест (`autolink_capped_at_enclosing_span_close`), +1 html (`…_capped_at_enclosing_monospace_span_close`).
    **Верификация:** clippy 0; test --workspace 0 упавших (parser 654→655, html_output 49→50); gate 344 **0 diff**; свип 860
    (4 корпуса) — изменился **ТОЛЬКО** wsl.adoc; семантически docs Identical **207→208** (wsl.adoc байт-identical asciidoctor).
    Коммит `70e1280`. **Merge/push — ПО ЗАПРОСУ.**

- [x] **F-BS. Auto-id секции из ФОРМАТИРОВАННОГО заголовка строился по сырому markup, а не по видимому тексту** (ветка
  `fix/section-id-from-formatted-title`, 2026-06-23). Запрос «начни следующую задачу». master `c24af34` (F-BR смержен).
  **Триаж:** кандидат session.md — `cheatsheet.adoc` (`/mnt/c/Work/docs/rest_api/cheatsheet…/`, 659 diff, single-root, 0 include).
  showdiff выявил 3 НЕЗАВИСИМЫХ дефекта: (1) **section-id** — доминирующий, 9 секций; (2) `ifdef::backend-html5[]` не вычислялся
  (passthrough-секция выпадала → хвост рассинхронизировался позиционным differ'ом, см. [[feedback_frontier_triage]]) — выделено
  в **F-BT**; (3) PlantUML `@dot/@enddot` блок (~125 diff остаток) — выделено в **F-BU**. **Корень F-BS (verified asciidoctor
  2.0.23):** Asciidoctor генерирует id из *подставленного* (`apply_title_subs`) заголовка, у которого `InvalidSectionIdCharsRx`
  затем срезает HTML-теги → инлайн-разметка вносит видимый текст, но НЕ маркеры: `[underline]#Basic formats#` → `_basic_formats`
  (было `_underlinebasic_formats`), `_Sidebar_ block` → `_sidebar_block` (было `__sidebar_block`), `*Bold*`/`` `code` ``/
  `Super^script^` → `_bold_…`/`_a_code_…`/`_superscript_…`. **Фикс (1 файл, only adoc-parser/block.rs):** в `generate_title_id`
  добавлен шаг `strip_inline_formatting` — прогон инлайн-парсера с subs=QUOTES и склейка текстовых leaf-ов (Text/Code/
  InlinePassthrough; SoftBreak/HardBreak→пробел); macros/URL/icon остаются литералом для `generate_id`'s `strip_urls_for_id`,
  типографика (`apply_typographic_replacements`) — следом, как раньше. Fast-path: заголовок без маркеров `*_`#^~[`` возвращается
  borrowed (без парсинга). +1 parser-тест `test_section_id_strips_inline_formatting` (6 кейсов), +1 html-тест
  `test_section_id_strips_inline_formatting_html`. clippy `--workspace` 0; **test --workspace 0 упавших** (html 543→**544**,
  parser 650→**651**, compat 233/233, html-compat 47, integration 29, render-core 25). **БАЙТ-НЕЙТРАЛЬНО:** база `/tmp/adoc_base`
  пересобрана от master `c24af34` (md5 `447ced4`), new `5b7323a`; gate 344 — **0 diff**; **свип 860 файлов — изменился ТОЛЬКО 1
  (целевой `cheatsheet.adoc`)**. Семантически: cheatsheet все heading-id ТЕПЕРЬ совпадают с asciidoctor (diff 659→125 при
  доп. `-a backend-html5`; остаток 125 = F-BU PlantUML). Коммит на ветке; merge/push — ПО ЗАПРОСУ.

- [x] **F-BT. `ifdef::backend-html5[]` / семейство backend-intrinsics не сеялось CLI** → условные блоки для html-бэкенда
  выпадали (в cheatsheet выпадала вся passthrough-секция, хвост рассинхронизировался) (ветка `fix/cli-seed-backend-intrinsics`,
  2026-06-23). Запрос «начни следующую задачу». master `bdb5355` (F-BS смержен). **Триаж (verified asciidoctor 2.0.23):**
  Asciidoctor при html5-рендере ставит `backend=html5`, `backend-html5` (set), `basebackend=html`, `basebackend-html`,
  `filetype=html`, `filetype-html`, `outfilesuffix=.html`, `doctype=article`, `doctype-article`. Наш `adoc-cli` сеял
  docname/docdate-семейство, но НЕ backend. Проба через CLI подтвердила: `ifdef::backend-html5[]`→NO, `{basebackend}`/
  `{filetype}`/`{outfilesuffix}` оставались литералом. **Корень:** `ifdef` вычисляется в препроцессоре (`preprocess_with_attrs`,
  до html-рендера) — а CLI не клал backend-intrinsics в `initial_attrs`; html-слой сеет `backend`/`doctype` в `document_attrs`
  только для inline `{backend}`. **Фикс (1 файл, only adoc-cli/main.rs):** через существующий `seed`-хелпер (кладёт в
  `initial_attrs` ДЛЯ препроцессора + `html_attrs` ДЛЯ рендера) засеяны `backend=html5`, `backend-html5`(set),
  `basebackend=html`, `basebackend-html`(set), `filetype=html`, `filetype-html`(set), `outfilesuffix=.html`. **`doctype-<value>`
  СОЗНАТЕЛЬНО НЕ сеется:** суффикс трекает значение doctype, которое header `:doctype:` может сменить (проба `:doctype: book` →
  asciidoctor даёт `doctype-book`, не `doctype-article`), а intrinsic мы не пересчитываем → жёсткий `doctype-article` оставил бы
  его ложно set для book/manpage. seed НЕ локает → header `:outfilesuffix: .adoc` по-прежнему побеждает (проба ✓). +2 cli
  integration-теста (`adoc-cli/tests/cli.rs`, новый файл: `seeds_backend_intrinsics` + `header_can_override_outfilesuffix`,
  гоняют бинарь через `CARGO_BIN_EXE_adoc`). clippy `--workspace` 0; **test --workspace 0 упавших** (html 544, parser 651,
  cli **+2**, compat 233, html-compat 47, integration 29, render-core 25). **БАЙТ-НЕЙТРАЛЬНО (где ожидалось):** база
  `/tmp/adoc_base` пересобрана от master `bdb5355` (md5 `5b7323a`), new `0b70065`; gate 344 — **0 diff**; **свип 860 файлов —
  изменился ТОЛЬКО 1 (целевой `cheatsheet.adoc`)** (backend-html5 в корпусах редок). Семантически (vs asciidoctor 2.0.23):
  cheatsheet **659→125 БЕЗ ручного `-a backend-html5`** — элементы 0..1055 теперь идентичны (passthrough/backend-html5 секции
  больше не выпадают, все heading-id совпадают); первое расхождение [1056] = PlantUML `@dot/@enddot` (F-BU). Set-сравнение
  (ref 1192 / our 1198 элементов): по существу различаются ~6 элементов, ВСЕ = F-BU (PlantUML/диаграммы + контекст: `#...#`
  highlight внутри passthrough — базовый `#mark#` сам по себе == asciidoctor; literal-дерево) — НЕ внесены F-BT. Коммит
  `200867c` на ветке; merge/push — ПО ЗАПРОСУ.
  - [ ] **Остаток doctype-intrinsics (отложенный краевой кейс):** `ifdef::doctype-book[]`/`doctype-manpage[]`/`doctype-inline[]`
    не работают — нужно пересчитывать intrinsic `doctype-<value>` при смене `:doctype:` (в шапке/CLI), а не сеять статикой.
    Архитектурно: либо логика в препроцессоре (отслеживает `doctype`, ставит/снимает суффикс-флаг), либо doctype-aware seed.
    Малочастотный; вне cheatsheet. Отдельная задача.

- [x] **F-BU. Литеральный (indented) параграф обрывался на первой НЕ-indented строке** → хвостовые строки отваливались в
  отдельный параграф, а одиночный собранный opener терял ведущие пробелы (ложный common-indent) (ветка
  `fix/literal-paragraph-continuation`, 2026-06-24). Запрос «начни следующую задачу». master `de8d3fa` (F-BT смержен).
  **Метка прошлой сессии «PlantUML @dot/@enddot» была ОШИБОЧНОЙ** — диаграммного расширения нет, asciidoctor рендерит
  `[uml]` open-блок как обычные параграфы; реальный корень — блочное правило литерального параграфа. **Триаж (showdiff
  cheatsheet + 12 минимальных проб vs asciidoctor 2.0.23):** в source-стр. 352-365 внутри open-блока `--` контент
  `@startdot…` с пустой строкой, после которой indented `  node1 -> node2 -> node3` стартует литеральный параграф, а
  следующие НЕ-indented `}` и `@enddot` (без пустой строки между) asciidoctor поглощает в ТОТ ЖЕ `<pre>` (с сохранением
  ведущих пробелов первой строки), мы же рвали на `}`. **Правило (verified):** литеральный параграф продолжается на все
  смежные непустые строки независимо от отступа — обрыв ТОЛЬКО на пустой строке / делимитере блока (`----`,`====`,`....`,
  `--`,`++++`,`____`,`****`,`////`) / md-fence / table `|===` / block-attr `[...]` / list-continuation `+`; (в list-контексте
  ещё list/dlist/callout-маркеры). НЕ обрывают (поглощаются): не-indented текст, секция `== T`, admonition `NOTE:`,
  list-маркер вне списка, line-comment `//`. Это в точности asciidoctor `StartOfBlockProc`. Индентация стрипается уже
  корректно (общий минимум). **Фикс (1 файл, only adoc-parser/block.rs):** в `scan_literal_paragraph` заменён цикл-сборщик
  (был `break` на любой не-indented-не-comment строке) на правильный набор терминаторов; guard `!lines.is_empty()` гарантирует
  поглощение opener'а (защита от одиночного indented `+`, который `is_list_continuation` ловит через `trim`). +1 parser-тест
  `test_literal_paragraph_absorbs_following_lines` (3 кейса: поглощение + min-indent strip + обрыв на делимитере), +1 html-тест
  `test_literal_paragraph_absorbs_following_lines_html`. clippy `--workspace` 0; **test --workspace 0 упавших** (parser 651,
  html 544, integration 29→**30**, compat 233/233, html-compat). **БАЙТ-НЕЙТРАЛЬНО:** база `/tmp/adoc_base` пересобрана от
  master `de8d3fa` (md5 `0b70065`); gate 344 — **0 diff**; **свип 860 файлов — изменился ТОЛЬКО 1 (целевой cheatsheet)**.
  Семантически: cheatsheet **125→58** позиционно; set-diff (Counter) base 13→**7** уникальных, причём все 7 присутствовали и в
  базе — фикс удалил ровно 6 расходящихся элементов (весь dot-diagram split: `node1…`, `}\n@enddot`, лишние `<p>/<div paragraph>`)
  и **НЕ добавил ни одного нового**. Коммит `1b18515` на ветке; merge/push — ПО ЗАПРОСУ.
  - [ ] **F-BV (остаток cheatsheet 58 = отдельный INLINE-слой, НЕ блочный): `#…#` mark/highlight внутри `[tree]` open-блоков**
    (source-стр. 427-441 и 448+). Контент `#booksample\n##asciidoctor.css\n…` — один параграф; asciidoctor гоняет
    constrained `#…#` / unconstrained `##…##` через soft-break'и и выдаёт сам артефакт (`#</mark>#kodcu.png`) — это
    tree-extension-разметка, не настоящий mark. Низкоценно (garbage-in), архитектурно — string-rewriting inline-движок
    ([[proj_sequential_quotes_rewrite]]), глубокий corner `#`/`##` через мультистрочный параграф. Также во втором tree-блоке
    ASCII-дерево `root\n|-- …` разбивается иначе. Отдельная задача, малочастотно.

- [x] **F-BR. Заголовок секции внутри list-continuation: строка вида `=== T` (ATX) или setext, присоединённая к элементу
  списка через `+`, ошибочно создавала секцию вместо литерального параграфа** (ветка `fix/section-title-in-list-continuation`,
  2026-06-23). Запрос «начни следующую задачу». master `c5f0d8b` (F-BQ смержен). **Триаж:** session-метка прошлой сессии
  (`_responses`/`_artifacts` в `http-api-design/src/main/asciidoc`) УСТАРЕЛА — те файлы уже 0-diff. Свежий `frontier_parity.py
  /mnt/c/Work/docs` → 202 identical, 7 clean; реальные кандидаты d=8 — `.deploy`-версии (`http-api-design.deploy/src/main/adoc/
  _responses.adoc` + `_artifacts.adoc`). **Корень (verified asciidoctor):** секция НИКОГДА не может быть ребёнком list-item;
  при list-continuation `+` следующий блок парсится в контексте элемента, и section-title (ATX `=== T` ИЛИ setext) там
  ДЕГРАДИРУЕТ в литеральный параграф `<p>=== T</p>` внутри `<li>` (пробы A/C/F/G + S_plus1/S_tilde3 vs asciidoctor 2.0.23:
  `+` setext → sect4/h5 на section-level, но в continuation — параграф). Тип триггера — наш флаг `in_continuation`. **Фикс
  (1 файл, only adoc-parser/block.rs):** guard `!self.in_continuation` на ОБА детекта в `scan_leaf_blocks` (ATX
  `strip_any_section_marker` + setext-ветка) → строка проваливается в paragraph-fallback. +1 parser event-тест
  `test_list_continuation_demotes_section_title`, +1 html-тест `test_section_title_demoted_in_list_continuation_html`. clippy
  `--workspace` 0; **test --workspace 0 упавших** (html 542→**543**, parser 649→**650**, compat 233, html-compat, integration
  29, render-core 25). **БАЙТ-НЕЙТРАЛЬНО:** база `/tmp/adoc_base` пересобрана от master `c5f0d8b` (md5 `1fed8ba`), new
  `447ced4`; gate 344 — **0 diff**; **свип 860 файлов — изменились 5: 2 целевых `.deploy` + 2 vue (`vue_ts_springboot` ×2,
  setext-shaped `на`+`+`) + родитель `http-api-design.adoc` (через include)**. Все улучшились: docs Identical 202→**206**
  (4 флипа: 2 `.deploy` + 2 vue к 0-diff), родитель 3926→**3712** diff-lines. **Важно:** vue флипнул не через ATX, а через
  setext-демотацию (`на` над `+`-маркером continuation = level-4 setext на section-level; позиционный differ раздувал
  один upstream-рассинхрон до 273 — классика [[feedback_frontier_triage]]). Коммит `b522081` на ветке; merge/push — ПО ЗАПРОСУ.

- [x] **F-BQ. Closed-ATX заголовок секции: закрывающий run маркеров (`== Title ==`, `==== T ====`, `## MD ##`) не
  отбрасывался — оставался литералом в тексте заголовка и портил auto-id** (ветка `fix/closed-atx-section-title`,
  2026-06-23). Запрос «начни следующую задачу». master `8dc6539` (F-BP смержен; session.md прошлой сессии устарел — писался
  до коммита/мержа F-BP). **Триаж:** notes/frontier/adoc2docx исчерпаны на чистых блочных фиксах (notes 79 id — остаток 2
  архитектурных автолинка; frontier 230 id — manpage др. бэкенд + `{asciidoctor-version}` интринсик + localtime; adoc2docx
  45 id — остаток = rouge-подсветка ruby/yaml, крупная фича). **РАСШИРЕН КОРПУС: `frontier_parity.py /mnt/c/Work/docs`
  (96 .adoc вне notes, ранее не майнились) → 200 identical, 9 чистых расхождений.** Выбран самый чистый single-root,
  флипающий 2 файла: **Release.adoc (5) + BranchesInPTS.adoc (7)**. **Баг:** исходники в «closed-ATX» стиле (`= T =`,
  `== T ==`, … `==== T ====`); asciidoctor отбрасывает закрывающий run, равный ведущему, мы оставляли как текст (`Выпуск
  версии ==`) + портили id (`_title_` вместо `_title`). **Корень (verified asciidoctor `SectionTitleRx`
  `/^((?:=|#){1,6})(?!\1[=#])\s+(\S.*?)(?:\s+\1)?$/`):** опциональная группа `(?:\s+\1)?` — трейлинг run, ТОЧНО равный
  ведущему (тот же символ + count), с whitespace перед ним, отбрасывается; асимметричный (`== T =`) или беспробельный
  (`== T==`) трейлинг — литерал. **Фикс (1 файл, only adoc-parser/scanner.rs):** новый хелпер `strip_closed_atx_trailer(
  title, marker, level)` вызывается в `strip_section_marker` (marker `=`) и `strip_markdown_heading` (marker `#`) после
  trim, возвращает суб-слайс (zero-copy). +1 scanner unit-тест `test_strip_closed_atx_section_title` (симметричный/
  асимметричный/беспробельный/`=` внутри title/markdown), +1 html-тест `test_closed_atx_section_title_html`. clippy
  `--workspace` 0; **test --workspace 0 упавших** (html 541→**542**, parser 649, compat 233, render-core 25, integration 29,
  html-compat). **БАЙТ-НЕЙТРАЛЬНО:** база `/tmp/adoc_base` пересобрана от master `8dc6539`; gate 344 — **0 diff**
  (`gate_check.py`); **свип всех корпусов (860 файлов: gate+frontier+adoc2docx+docs) — изменились ТОЛЬКО 2 целевых файла**.
  Семантически без регрессий: frontier 230, adoc2docx 45, notes 79 стабильны; docs Identical 200→**202**. 5 CLI-проб
  байт-в-байт == asciidoctor 2.0.23 (`Title`/`Asym =`/`master` h4/`NoSpace==`/`MD Two`). Коммит/merge/push — ПО ЗАПРОСУ.
  - [x] ~~`_responses.adoc`/`_artifacts.adoc` (8) + vue_ts_springboot ×2 (273/270)~~ — ЗАКРЫТО F-BR (section-title в
    list-continuation; vue флипнул через setext-демотацию, не ATX).
  - [ ] **Свежий остаток docs-корпуса (кандидаты след. сессий):** cheatsheet (659 — крупный, требует showdiff-триажа на
    single/multi-root). windows/wsl (95) + keycloak/index (52) — те же 2 архитектурных автолинка (`macros` до
    `quotes`/specialchars, реордер; НЕ single-session, см. [[proj_sequential_quotes_rewrite]]).

- [x] **F-BP. Блок-атрибут (`[source]`/`[#id.role]`/`[quote]`), примыкающий к list-item БЕЗ пустой строки, ошибочно
  применялся к следующему делимитед-блоку вместо отбрасывания** (ветка `fix/list-discards-abutting-block-attr`,
  2026-06-23). Запрос «начни следующую задачу». master `bccc15f` (F-BO смержен). **Триаж notes
  (`frontier_parity.py /mnt/c/Work/docs/notes/modules`): 78 identical, 3 чистых расхождения.** Из трёх остаток-кандидатов
  windows/wsl (95) и keycloak/index (52) — АРХИТЕКТУРНЫЕ автолинк-кейсы (у нас `macros::extract` идёт ДО `quotes` и до
  экранирования спецсимволов, отложенного в рендер; у asciidoctor `specialchars → quotes → macros`, поэтому к моменту
  автолинка backtick уже `<code>` (его `<` обрывает URL) и `<>` уже `&lt;`) — НЕ чистый single-session фикс. Выбран
  **ansible/tips (41)** — единственный чистый блочный корень. **Баг:** `. Пример выравнивания слева` + сразу (без пустой
  строки) `[source, yaml]` + `----` listing — asciidoctor роняет source-роль → plain `<pre>`, у нас оставался
  `<pre class="highlight"><code class="language-yaml">`. **Корень (verified `parser.rb` 2.0.23
  `read_lines_for_list_item`):** строки 1499-1501 — block-метаданная строка (`[...]`/`.Title`/`:attr:`), примыкающая к
  list-item, читается в буфер строк элемента (`buffer << this_line`); строки 1453-1456 — «a delimited block immediately
  breaks the list unless preceded by a list continuation»: делимитед-блок БЕЗ предшествующего `+` ломает список, оставляя
  метаданную без блока внутри элемента для привязки → asciidoctor её ОТБРАСЫВАЕТ, а блок парсится заново на верхнем уровне
  БЕЗ неё. **Pending-атрибуты в открытом списке возможны ТОЛЬКО при примыкании** (без пустой строки) — иначе blank-ветка
  (`scan_header_constructs` 912/936) закрыла бы список в момент сканирования `[...]`/`.Title`. **Фикс (1 файл, only
  adoc-parser/block.rs):** новый хелпер `discard_list_absorbed_metadata` (обнуляет `pending_block_attrs` +
  `pending_block_title`) вызывается в путях делимитед-блока (`scan_block_containers` ~1247) и markdown-fence (~1261) перед
  `close_list_contexts`, в ветке `is_directly_in_list_context() && !in_continuation`. **`+`-continuation «спасает»
  атрибут** (`in_continuation` исключает ветку → атрибут доживает до вложенного блока, как E7). +1 html-тест
  `test_list_discards_abutting_block_attr_html` (adoc-html: `[source]`/`[#id.role]`/`[quote]` примыкание → отброшены;
  `.Title` примыкание вливается в параграф как текст, НЕ block-title div; регрессии: `+`-continuation держит+вкладывает,
  пустая строка держит, top-level держит). clippy `--workspace` 0; **test --workspace 0 упавших** (html 540→**541**,
  parser 648, compat 233, render-core 25, integration 29, html-compat). **БАЙТ-НЕЙТРАЛЬНО на старых корпусах:** gate 344 —
  **0 diff** vs base `bccc15f` (`gate_check.py`), frontier(250)+adoc2docx(52)=302 — **0 diff** (`/tmp/sweep_bvn.py`).
  **notes Identical 78→79** (ansible/tips → identical). 10+ CLI-проб == asciidoctor 2.0.23 (`[source]`/`.Title`/`[#id.role]`/
  `[quote]` примыкание отброшены; `____` остаётся quoteblock нативно; `+`-continuation и blank-line держат роль; bonus —
  `[quote]`+`----` теперь plain listing, раньше у нас ошибочно quoteblock). Коммит/merge/push — ПО ЗАПРОСУ.
  - [ ] **Остаток notes (2 архитектурных расхождения, требуют рерайта порядка субституций macros↔quotes):** windows/wsl
    (95, автолинк URL внутри backtick-кода захватывает закрывающий backtick в href), keycloak/index (52, автолинк URL с
    `<host>:<port>` внутри inline-кода не срабатывает — `<` обрывает URL до экранирования). ОБА — следствие `macros` до
    `quotes` в нашем движке (см. proj_sequential_quotes_rewrite); чистого блочного остатка в notes больше НЕТ.
  - **Пре-существующие дивергенции (НЕ в scope F-BP, отдельный феномен поглощения параграфом):** `. item`+`[.role]`+
    `параграф` (asciidoctor вливает «параграф» в текст элемента, у нас sibling-параграф с role); `. item`+`[.role]`+`NOTE:`
    (asciidoctor вливает `NOTE:` как литерал, у нас вложенный admonition). Это правило `read_paragraph_lines` (block-title и
    инлайн-текст не прерывают параграф list-item), родственно F-BO, НЕ затронуто фиксом delimited-пути.

- [x] **F-BO. Список НЕ прерывает параграф: маркер `*`/`-`/`.`/`term::` сразу за текстом параграфа (без пустой
  строки) ошибочно стартовал список вместо поглощения в параграф** (ветка `fix/list-no-interrupt-paragraph`,
  2026-06-23). Запрос «начни следующую задачу». master `c8bf4ed` (F-BN смержен). **Триаж notes
  (`frontier_parity.py /mnt/c/Work/docs/notes/modules`): 77 identical, 4 чистых расхождения.** Выбрано
  `projects/pages/sbertech/index.adoc` (134 token-diff) — самый высокоимпактный И самый чистый КОРЕНЬ
  (один десинк рушит весь хвост документа). **Баг:** `Устанавливаем … Ручная установка.` + сразу `. Выполняем
  раздел "Подготовка DTL".` (без пустой строки) — asciidoctor ПОГЛОЩАЕТ маркер в параграф (`<p>… . Выполняем …</p>`),
  у нас `. Выполняем` стартовал список и весь хвост десинкался. **Корень (verified `parser.rb` 2.0.23):**
  `read_paragraph_lines reader, break_at_list = (skipped == 0 && options[:list_type])` — список прерывает параграф
  ТОЛЬКО внутри списочного контекста; на верхнем уровне (`list_type` nil) маркер любой семьи (`AnyListRx`:
  `*`/`-`/`.`/`term::`) сразу за текстом параграфа = обычный текст, НЕ список. Наш `scan_paragraph`/`scan_admonition`
  ломали параграф на маркере БЕЗУСЛОВНО. **Фикс (1 файл, only adoc-parser/block.rs):** в обоих циклах чтения параграфа
  (`scan_paragraph` + `scan_admonition`) break на `is_list_marker_unordered`/`is_list_marker_ordered`/
  `is_description_list_marker` обёрнут в `self.is_directly_in_list_context()` (callout уже был под `is_in_callout_list`).
  **`is_directly_in_list_context` (НЕ `is_in_list_context`)** — точный аналог `options[:list_type]`: возвращает false,
  когда между нами и list-item на стеке стоит delimited-block (asciidoctor парсит тело такого блока в свежем контексте,
  без `list_type`) → внутри `--`/`====` вложенного в список маркер тоже поглощается (пробы подтвердили). +1 html-тест
  `test_list_does_not_interrupt_paragraph_html` (ordered/unordered/dlist/admonition поглощение + регрессии: список после
  пустой строки стартует, вложенность работает). clippy `--workspace` 0; **test --workspace 0 упавших** (html 540, parser
  648, compat 233, render-core 25, integration 29, html-compat; всего 1307 passed). **БАЙТ-НЕЙТРАЛЬНО на старых корпусах:**
  gate 344 — **0 diff** vs base `c8bf4ed` (`gate_check.py`), frontier(250)+adoc2docx(52)=302 — **0 diff** (`sweep_bvn.py`).
  **notes Identical 77→78** (sbertech → identical). 12+ CLI-проб == asciidoctor 2.0.23 (поглощение `*`/`-`/`.`/`term::`/
  admonition; open-block-в-списке поглощает; регрессии list-first/после-blank/nested/callout/continuation-sibling).
  **Пре-существующая дивергенция (НЕ в scope, идентична master):** continuation-параграф (`+`) с ГЛУБЖЕ-вложенным
  маркером (`.. nested` под `.`-списком) — asciidoctor поглощает, мы стартуем nested-список (отдельное правило
  list-continuation). Коммит/merge/push — ПО ЗАПРОСУ.
  - [ ] **Остаток notes (3 чистых расхождения, кандидаты на следующие сессии):** windows/wsl (95, автолинк URL захватывает
    закрывающий backtick), keycloak/index (52, автолинк URL с `<host>` внутри inline-кода не срабатывает), ansible/tips
    (41, `[source,yaml]` примыкает к list-item без пустой строки → asciidoctor теряет source-роль, у нас highlighted).

- [x] **F-BN. Unordered-список: Unicode-маркер `•` (U+2022) не распознавался** (ветка `fix/bullet-unordered-marker`,
  2026-06-23). Запрос «начни следующую задачу». master `5b9026c` (F-BM смержен). **Триаж notes
  (`frontier_parity.py /mnt/c/Work/docs/notes/modules`): 76 identical, 5 чистых расхождений.** Выбрано
  `projects/ai/plan.adoc` (230 token-diff) — самое чистое ПРАВИЛО (не самый маленький diff). **Баг:** строки
  `• Сократить … / • Получить … / …` у нас оставались литералом в одном `<p>`, у asciidoctor → `<ul><li>`.
  **Корень (verified `rx.rb` + пробы asciidoctor 2.0.23):** asciidoctor `UnorderedListRx = /^[ \t]*(-|\*\**|•)[ \t]+…/`
  — `•` равноправный unordered-маркер наряду с `-`/`*`; `is_list_marker_unordered` (scanner.rs) принимал только `-` и `*`.
  `•` — отдельная СЕМЬЯ маркеров (ровно один `•`, `••` НЕ маркер; нестится независимо от `-`/`*` — пробы `* a`+`• b`
  и наоборот). **Фикс (1 файл, only adoc-parser/scanner.rs, ~5 строк логики):** в `is_list_marker_unordered` добавлен
  bail на `strip_prefix('\u{2022}')` + `marker_content` (обеспечивает `[ \t]+`, отсекает `••`/`•no-space`) → идентичность
  255 (out of band ВЫШЕ star-счётчиков, зеркало hyphen=0 ниже; depth для unordered `ListItem` отбрасывается в HTML, 255
  не утекает). +4 scanner-assert'а в `test_is_list_marker_unordered`, +1 html-тест `test_unordered_bullet_marker_html`
  (плоский `•`-список, нестинг под `*`, regression `••`/`•no-space`→параграф). clippy `--workspace` 0;
  **test --workspace 0 упавших** (html 538→**539**, parser 648, compat 233, render-core 25, integration 29, html-compat 1).
  **БАЙТ-НЕЙТРАЛЬНО на старых корпусах:** gate 344 — **0 diff** vs base `5b9026c` (`gate_check.py`),
  frontier(250)+adoc2docx(52)=302 — **0 diff** (`sweep_bvn.py`) — `•`-маркер там не встречается. **notes Identical 76→77**
  (plan → identical). 7 CLI-проб (`•`-список/`* a`+`• b`/`• a`+`* b`/`••`/`•no-space`/`•\t`/`text • mid`) == asciidoctor
  2.0.23 байт-в-байт. Коммит/merge/push — ПО ЗАПРОСУ.
  - [ ] **Остаток notes (4 чистых расхождения, кандидаты на следующие сессии):** sbertech/index (134, ordered-list
    `.`-десинк), windows/wsl (95, автолинк URL захватывает закрывающий backtick), keycloak/index (52, автолинк URL с
    `<host>` внутри inline-кода не срабатывает), ansible/tips (41, `[source,yaml]` примыкает к list-item без пустой строки
    → asciidoctor теряет source-роль, у нас остаётся highlighted).

- [x] **F-BM. Sub/superscript: `~…~` / `^…^` ложно срабатывали на содержимом с пробелами** (ветка
  `fix/subscript-superscript-whitespace`, 2026-06-23). Запрос «начни следующую задачу». master `6a39972` (F-BL смержен).
  **Триаж notes (`frontier_parity.py /mnt/c/Work/docs/notes/modules`): 75 identical, 6 чистых расхождений.** Выбрано
  `projects/ai/qwen.adoc` (194 token-diff) — самое чистое правило. **Баг:** текст `При INT4 (~33 ГБ VRAM) … ~40%` у нас
  превращался в `(<sub>33 ГБ VRAM) обеспечивает </sub>40%`, у asciidoctor остаётся литералом. **Корень (verified пробами
  asciidoctor 2.0.23):** asciidoctor `SubscriptRx`/`SuperscriptRx` = `~(\S+?)~` / `^(\S+?)^` — содержимое НЕ должно
  содержать whitespace; `simple_pair_open_close` (subst/quotes.rs — активный string-rewriting движок, НЕ legacy
  inline.rs!) брал первый закрывающий маркер без проверки пробелов. **Фикс (1 строка логики в `simple_pair_open_close`):**
  bail `None` на первом `is_ascii_whitespace()` байте перед закрывающим маркером. Проверка на СЫРЫХ байтах ДО прохода
  attributes (quotes идут раньше attributes), поэтому `^a{sp}b^`→`<sup>a b</sup>` (literal `{sp}`, нет whitespace-байтов)
  и passthrough-сентинел `^+a b+^`→`<sup>a b</sup>` (opaque, пропускается = non-whitespace) — оба сохранены. +1 тест
  `subscript_superscript_require_non_whitespace_content` (subst::tests, активный движок). clippy `--workspace` 0;
  **test --workspace 0 упавших** (parser 647→**648**, html 538, compat 233, render-core 25, integration 29, html-compat 1).
  **БАЙТ-НЕЙТРАЛЬНО на старых корпусах:** gate 344 — **0 diff** vs base `6a39972` (`gate_check.py`),
  frontier(250)+adoc2docx(52)=302 — **0 diff** (`sweep_bvn.py`). **notes Identical 75→76** (qwen → identical).
  4 CLI-пробы (`~33 ГБ~`/`H~2~O`/`^a{sp}b^`/`^+a b+^`) == asciidoctor 2.0.23 байт-в-байт. Коммит/merge/push — ПО ЗАПРОСУ.
  - [ ] **Остаток notes (5 чистых расхождений):** projects/ai/plan (230, `•`-bullet-маркеры), sbertech/index (134,
    ordered-list `.`-десинк), windows/wsl (95, автолинк URL захватывает закрывающий backtick), keycloak/index (52,
    автолинк URL с `<host>` внутри inline-кода не срабатывает), ansible/tips (41, `[source,yaml]` примыкает к list-item
    без пустой строки → asciidoctor теряет source-роль, у нас остаётся highlighted).

- [x] **F-BL. Список: TAB-разделитель после маркера (`.\tItem` / `*\tItem` / `-\tItem` / `1.\tItem`) не распознавался;
  `.\t…` дополнительно ловился как block-title** (ветка `fix/list-marker-tab-separator`, 2026-06-23). Запрос «начни
  следующую задачу». master `c9a9b58` (F-BK смержен). **Триаж notes-корпуса (`/mnt/c/Work/docs/notes/modules/`, 81 .adoc):
  74 identical, 7 чистых расхождений.** Самое чистое (23 token-diff) — `sbertech/synapse.adoc`: ordered-список с маркером
  `.` + **TAB** (`.\tПолучение данных из SOAP сервиса`) у asciidoctor → `<ol class="arabic">`, у нас весь список тихо
  пропадал. **Корень (2 функции в scanner.rs, verified `rx.rb` + пробы asciidoctor 2.0.23):** asciidoctor `AnyListRx`
  разделяет маркер и текст через `[ \t]` (пробел ИЛИ таб), а `is_list_marker_unordered`/`is_list_marker_ordered`
  принимали ТОЛЬКО пробел (`!rest.starts_with(' ')`). Дополнительно `is_block_title` (`BlockTitleRx ^\.([^\s.].*)$`)
  исключал лишь пробел после `.` → `.\t…` ловился как block-title в `scan_header_constructs` (раньше list-детектора) и
  список исчезал (поэтому `.\tItem` в начале документа падал, тогда как `*\t`/`-\t`/`1.\t` доходили до list-детектора).
  **Фикс (1 файл, only adoc-parser/scanner.rs):** новый хелпер `marker_content(rest)` = `strip_prefix(' ')`/`'\t'` →
  `trim_start` → non-empty (зеркало `[ \t]`); обе list-функции переведены на него; `is_block_title` переписан под
  `[^\s.]` (первый символ после `.` не whitespace и не `.`, через `is_whitespace()`). +6 scanner-assert'ов (3 функции),
  +1 html-тест `test_list_marker_tab_separator_html` (`.\t`/`1.\t`/`*\t`/`-\t` + regression `.Title` остаётся block-title).
  clippy `--workspace` 0; **test --workspace 0 упавших** (html 537→**538**, parser 647, compat 233, render-core 25,
  integration 29). **БАЙТ-НЕЙТРАЛЬНО на старых корпусах** (паттерн «маркер+TAB» там отсутствует, `grep` = 0 файлов):
  gate 344 — **0 diff** vs base `c9a9b58` (`gate_check.py`), frontier(250)+adoc2docx(52)=302 — **0 diff** (`sweep_bvn.py`).
  **notes Identical 74→75** (synapse → identical). 5 CLI-проб (`.\t`/`*\t`/`-\t`/`1.\t`/`..\t`) == asciidoctor 2.0.23
  байт-в-байт (тело). Коммит/merge/push — ПО ЗАПРОСУ.
  - [ ] **Остаток notes (6 чистых расхождений, кандидаты на следующие сессии):** projects/ai/plan (230), projects/ai/qwen
    (194), sbertech/index (134), windows/wsl (95), keycloak/index (52), ansible/tips (41).

- [x] **F-BK. Ordered-список: утечка флага `had_blank_line` от ведущего блока → `+`-continuation цепляется к ВНЕШНЕМУ
  пункту вместо вложенного** (ветка `fix/ordered-list-blank-line-leak`, 2026-06-23). Запрос «начни следующую задачу».
  master `feb2e38` (F-BJ смержен). **Триаж notes-корпуса (`/mnt/c/Work/docs/notes/modules/`, 81 .adoc): 73 identical,
  8 чистых расхождений.** Самое чистое (15 token-diff) — `antora/pages/index.adoc`: NOTE-admonition через `+` после
  последнего пункта вложенного ordered-списка (`.. Копируем токен` + `+` + `NOTE:`) у asciidoctor сидит ВНУТРИ `<li>`
  внутреннего списка, у нас выпадал наружу к внешнему пункту. **Корень (bisect-матрица проб):** баг требует ОДНОВРЕМЕННО
  (1) ведущий блок (абзац/секция) перед списком и (2) вложенность ≥2. `skip_blank_lines` ставит `had_blank_line=true` на
  пустой строке между ведущим блоком и списком; `scan_unordered_list_item` СБРАСЫВАЛ флаг (стр. 3947), а
  `scan_ordered_list_item` — **НЕТ** → флаг утекал через все пункты до обработчика `+` (стр. 1351
  `if had_blank_line && is_in_list_context → close_nested_list_items` цепляет к самому ВНЕШНЕМУ пункту). При пустой строке
  ПРЯМО перед `+` оба движка корректно цепляют к внешнему (asciidoctor verified) — там blank ставит флаг заново, фикс это
  сохраняет. **Фикс (1 строка, only adoc-parser):** `block.rs` — добавлен `self.had_blank_line = false;` в конец
  `scan_ordered_list_item` (зеркало unordered). +2 html-теста (`test_ordered_nested_continuation_after_leading_block_html`
  + `..._blank_before_plus_stays_outer_html`). clippy `--workspace` 0; **test --workspace 0 упавших** (html 535→**537**,
  parser 647, compat 233, render-core 25). **БАЙТ-НЕЙТРАЛЬНО на старых корпусах:** gate 344 — **0 diff** vs base `feb2e38`
  (`gate_check.py`), frontier(250)+adoc2docx(52)=302 — **0 diff** (`sweep_bvn.py`) — паттерн (ведущий блок + вложенный
  ordered + `+`) там не встречается. **notes Identical 73→74** (antora/index.adoc → identical). Коммит/merge/push — ПО ЗАПРОСУ.
  - [ ] **Остаток notes (7 чистых расхождений, кандидаты на следующие сессии):** projects/ai/plan (230), projects/ai/qwen
    (194), sbertech/index (134), windows/wsl (95), keycloak/index (52), ansible/tips (41), sbertech/synapse (23).

- [x] **F-BJ. Авто-ID секции — из СУБСТИТУИРОВАННОГО заголовка (em-dash/ellipsis/`(C)` схлопывают разделители)**
  (ветка `fix/auto-id-substituted-title`, 2026-06-23). Запрос «начни следующую задачу». master `e188b95` (F-BI/doc-поправка
  смержены). **Триаж:** все три старых корпуса исчерпаны (gate 344/344; frontier 230/250 — остаток manpage-бэкенд +
  `{asciidoctor-version}`/`{localtime}` env-intrinsics; adoc2docx 45/52 — остаток Rouge-подсветка). **Расширил frontier
  на РЕАЛЬНЫЕ рабочие доки `/mnt/c/Work/docs/notes/modules/` (81 .adoc, 0 include) → 72 identical, 9 чистых расхождений.**
  Самое чистое (1 token-diff) — `fa/index.adoc`: `== Решение -- {product}` (`:product: FORSed Architect`) → asciidoctor ID
  `_решениеforsed_architect`, наш `_решение_forsed_architect`. **Корень (verified `section.rb`+`abstract_block.rb`+`rx.rb`):**
  asciidoctor `Section.generate_id` слугифицирует `Section#title` = `apply_title_subs(@title)` — заголовок ПОСЛЕ субституций.
  `InvalidSectionIdCharsRx` УДАЛЯЕТ entity/теги; спейсед-em-dash replacement ` -- `→`&#8201;&#8212;&#8201;` ПОГЛОЩАЕТ
  окружающие пробелы → «решение»+(3 entity удалены, пробелов нет)+«forsed» = `решениеforsed`. Мы генерили ID из СЫРОГО
  заголовка → ` -- ` оставались разделителями → `решение_forsed`. Затрагивает 3 паттерна (verified пробами обоих движков):
  spaced em-dash `x -- y`→`_xyz`, word em-dash `pre--post`→`_prepost`, ellipsis `a...b`→`_ab` (+ бонус `(C)`/`(R)`).
  **Фикс (2 файла, only adoc-parser):** `block.rs` новый хелпер `generate_title_id(&self, title)` = `resolve_title_attr_refs`
  → `inline::apply_typographic_replacements(&t, true, true)` → `scanner::generate_id`; 4 call-сайта (doc-header ×2, section,
  discrete-heading) переведены на него. **Ключ:** наша `apply_typographic_replacements` производит UNICODE-глифы
  (`\u{2014}`/`\u{2009}`/`\u{2026}`/`\u{200B}`/arrows), а `generate_id` уже пропускает все не-alnum/не-ASCII-разделители →
  глифы дропаются, спейсед em-dash даёт смежность — БЕЗ правок самой slugify. +1 html-тест
  `test_section_id_from_substituted_title_html` (spaced/word em-dash, ellipsis, `(C)`, attr+em-dash, regression
  lone-hyphen/triple-hyphen). clippy `--workspace` 0; **test --workspace 0 упавших** (html 534→**535**, parser 647,
  compat 233, render-core 25). **БАЙТ-НЕЙТРАЛЬНО на всех старых корпусах:** gate 344 — **0 diff** vs base `e188b95`
  (`gate_check.py`), frontier(250)+adoc2docx(52)=302 — **0 diff** (`sweep_bvn.py`) — паттерн em-dash/ellipsis в ЗАГОЛОВКАХ
  там не встречается. **notes Identical 72→73** (fa/index.adoc → identical). 23 CLI-пробы == asciidoctor 2.0.23.
  Коммит/merge/push — ПО ЗАПРОСУ.
  - [ ] **Остаток (отдельный класс, предсуществующий, base==new, вне scope F-BJ): формат-маркеры в заголовке.** `_em_`/`*b*`
    и т.п. в title: asciidoctor гонит QUOTES (→`<em>`, срезает тег) → `_em_ text`→`_em_text`; мы трактуем `_`/`#` как
    разделитель → `__em_text`. Это класс QUOTES (не REPLACEMENTS); корректный фикс требует прогонять quotes-пасс на title
    или различать литеральный `_` (snake_case) от italic-маркера — риск, не в notes-корпусе как чистый кейс. Дефернуто.

- [x] **F-BI. Пустая секция: пустая строка в content-слоте (`\n#{content}\n` для пустого тела)** (ветка
  `fix/empty-section-blank-line`, 2026-06-23). Запрос «начни следующую задачу». master `8001e22` (F-BH смержен).
  Триаж дефернутых остатков: 3 кандидата (mailto-курлинг, backtick-apostrophe `` `' ``, em-dash в inline-code) ОКАЗАЛИСЬ
  УЖЕ ИСПРАВЛЕНЫ (метки устарели); arrow-in-bare-URL — бой с архитектурой (наш порядок `macros`→`replacements`, у
  asciidoctor наоборот), риск. Взят чистый рендер-остаток.
  **Корень:** шаблон секции asciidoctor оборачивает контент в `\n#{content}\n`. Для НЕпустого тела завершающий `\n`
  даёт последний дочерний блок (`</div>\n` параграфа) — мы совпадали. Для ПУСТОГО тела этот `\n` отсутствовал: sect1
  давал `<div class="sectionbody">\n</div>` вместо `…\n\n</div>`, sect2+ — `</h3>\n</div>` вместо `…\n\n</div>`.
  **Фикс (3 файла, only adoc-html):** новый стек `section_content_start: Vec<usize>` (`lib.rs`) — push placeholder
  `usize::MAX` в `start_section_div` (`blocks.rs`), запись `output.len()` в `TagEnd::SectionTitle` (после `</hN>\n` и,
  для sect1, открытия `sectionbody`), в `TagEnd::Section` (гард `!is_sect0`) при `output.len()==content_start` (тело
  ничего не написало) добавляется `\n`. Заголовок документа идёт через `SectionTitle` но НЕ через `start_section_div`
  (он в header) → стек пуст → no-op, баланс сохранён. +1 html-тест `test_empty_section_emits_blank_content_slot_html`
  (sect1/sect2/comment-only пусто + regression непустой). clippy `--workspace` 0; **test --workspace 0 упавших**
  (html 533→**534**, parser 647, compat 233, render-core 25, html-compat зелёные — whitespace невидим семантическому DOM).
  **БАЙТ-ПАРИТЕТ (0 регрессий, 10 файлов улучшено, 7 → полная идентичность):** гейт(344) — abstract/part/outline →
  **identical**, appendix 9→1, section 18→2 (остаток = sect0); sweep frontier+adoc2docx(302) — include-with-leading-
  blank-line/multi-special-ex/sections/toc → **identical**, callouts 37→36 (остаток = Rouge). Закоммичен/смержен/запушен
  (master `27ba193`), решение «оставить».
  **⚠ ПОПРАВКА ПОЛЬЗОВАТЕЛЯ (2026-06-23):** F-BI — РАБОТА ВНЕ ЦЕЛИ. Для HTML байт-паритет нужен ТОЛЬКО в листингах/
  примерах (verbatim `<pre>`); пустая строка между `<div>`-тегами браузером схлопывается → на рендеринг не влияет,
  html-compat (семантический DOM) её игнорировал — и это ПРАВИЛЬНО. Нормализованная метрика НЕ «слепа» — она корректно
  не видит неважное. См. [[feedback_html_byte_parity_scope]]. **Поэтому sect0 blank-between-siblings (пустая строка
  между соседними `<h1 class="sect0">`, appendix/section residual) — НЕ кандидат на задачу** (тот же класс: whitespace
  вне `<pre>`). Будущий compat-триаж: байт-паритет преследовать только ВНУТРИ verbatim-блоков; всё прочее мерить
  семантикой DOM (`adoc-html-tests`), не байтами.

- [x] **F-BH. `(C)`/`(R)` курлятся рядом с буквой (снят `!followed-by-letter` guard)** (ветка
  `fix/copyright-registered-letter-adjacent`, 2026-06-23). Запрос «начни следующую задачу» → выбор «дефернутые остатки».
  master `85a7a72` (F-BG смержен). Корпуса исчерпаны: гейт 344/344, frontier 230/250 (остаток — manpage др. бэкенд +
  `{asciidoctor-version}` нерешаемо + `{localtime}` уже работает, артефакт кэша), adoc2docx 45/52 (4 файла — всё Rouge-
  подсветка, верифицировано: первый diff в каждом = `<span class="kd/nc/na/nb/s1">`). Взят дефернутый остаток F-BG(1).
  **Корень:** `apply_typographic_replacements` (`inline.rs`) гейтил `(C)`/`(R)` условием `!matches!(bytes.get(i+3),
  Some(letter))` — курлил только если за маркером НЕ буква. asciidoctor `REPLACEMENTS` `/\\?\(C\)/`/`/\\?\(R\)/` тип
  `:none` → замена ВЕЗДЕ, без контекстного гейта (как `(TM)`, у которого guard'а и не было). Проба asciidoctor 2.0.23:
  `a(C)b`→`a©b`, `x(R)y`→`x®y`, `prefix(C)suffix`→`prefix©suffix`. **Фикс (2 файла):** `inline.rs` — убран
  followed-by-letter guard из арм'ов `(C)` и `(R)` (единственная реализация — оба движка `subst/replacements.rs`+legacy
  делегируют сюда). Escaped `\(C)` запечатан escape-пассом в Literal ДО replacements → соседство с буквой его не
  воскрешает (проверено). +2 теста (`test_typographic_copyright_registered_letter_adjacent` parser,
  `test_copyright_registered_letter_adjacent_html` html, оба + escaped-гард). clippy `--workspace` 0; **test --workspace
  0 упавших** (parser 646→647, html 532→533, compat 233, render-core 25). **Гейт 344/344 байт-в-байт** vs master
  `85a7a72` (0 diff — паттерн `(C)`/`(R)`+буква не встречается в гейте). **Sweep frontier(250)+adoc2docx(52)=302
  new-vs-base: 0 расхождений** (дефернутый остаток — паттерн не в корпусах; метрика parity не двигается, реальная
  дивергенция от asciidoctor устранена). CLI-пробы (`a(C)b`/`x(R)y`/`m(TM)n`/`prefix(C)suffix`/escaped) == asciidoctor
  2.0.23 байт-в-байт (NCR-нейтрализация). Коммит/merge/push — ПО ЗАПРОСУ.

- [x] **F-BG. `replacements` в inline-link TARGET (`...`/`--`/… курлятся в href)** (ветка
  `fix/replacements-in-link-target`, 2026-06-23). Запрос «начни следующую задачу». master `a8bdd1c` (F-BF смержен).
  Триаж: frontier исчерпан до 3 single-diff (manpage — др. бэкенд; `{docdate}`/`{asciidoctor-version}` — env-intrinsics,
  нестабильны), adoc2docx — 4 крупных мульти-root (Rouge/sequential-quotes). Единственный стабильный кандидат —
  **`CHANGELOG.adoc` (1 token-diff, строка 1173)**: `compare/v1.5.6.1...v1.5.6.2[full diff]` — asciidoctor курлит `...`→
  `&#8230;&#8203;` в href, мы — нет. **Корень:** asciidoctor гонит `replacements` ДО `macros`, поэтому `...` в URL-таргете
  курлится до автолинковки; наш движок гонит `macros` ДО `replacements` (зеркало legacy), URL извлекается в leaf до
  replacements. **Фикс (4 файла, движок+рендерер):** `subst/macros.rs` `reconstruct_link_target` теперь прогоняет
  СЫРЫЕ сегменты таргета через `apply_typographic_replacements(seg,false,false)` (URL без пробелов → spaced em-dash
  невозможен; sealed Literal/CharRef сплайсятся verbatim → escaped `\...` остаётся литералом, как в asciidoctor `\\?`);
  helper `maybe_curl_link_target`; `try_link` no-sentinel ветка тоже через reconstruct; 4 caller'а прокидывают `options`.
  `inline.rs` — поле `InlineOptions.link_target_pre_substituted` (default false=курлим). `adoc-html/lib.rs`
  `render_inline_value_with_subs_flag`; `events.rs` combined `{attr}value[text]` реинлайн ставит флаг **true** → НЕ
  курлит повторно (escape `\...` потерян escape-до-attributes в pass 1; гард — тест 605 `test_escaped_ellipsis_in_link_target_html`).
  +2 теста (`unescaped_ellipsis_in_url_target_curls` parser, `test_unescaped_ellipsis_in_link_target_curls_html` html).
  clippy `--workspace` 0; **test --workspace 1297, 0 упавших** (html 531→532, parser 645→646, compat 233, render-core 25).
  **Гейт 344/344 байт-в-байт** vs master `a8bdd1c` (0 gate-URL с `...`/`--`). **Sweep frontier(250)+adoc2docx(52)=302
  new-vs-base: РОВНО 1 файл** (CHANGELOG), **0 регрессий**; **frontier Identical 229→230**, CHANGELOG token-identical.
  Пробы (link:/bare/URL[text] с `...`/`--`) == asciidoctor 2.0.23 (raw UTF-8 `…​`/`—​` == их `&#8230;&#8203;`/`&#8212;&#8203;`).
  Коммит/merge/push — ПО ЗАПРОСУ.
  - [ ] **Остаток (вне scope, пред-существующее, BASE==NEW):** (1) ~~`(C)`-после-буквы~~ → **СДЕЛАНО в F-BH ниже**;
    (2) **mailto/email** таргеты не курлятся
    (try_mailto строит url из base+encoded, не через reconstruct); (3) **resolved-attr `{u}/path...[t]`** (Document
    реинлайн) — unescaped `...` НЕ курлится (флаг подавляет ради защиты escaped, escape потерян в pass 1) → дивергенция
    от asciidoctor; (4) undefined-attr `\...` в trailing (MissingSkip path) курлит плейн-текстом; (5) `->`/arrow в bare
    URL ломает границу (URL-скан стопает на `>`) — отд. boundary-баг. Все рідкие, не в корпусе.

- [x] **F-BF. `[partintro]` вне book-части исключается (`convert_open` → '')** (ветка
  `fix/partintro-exclude-outside-book`, 2026-06-23). Запрос «начни следующую задачу». master `afef333` (F-BE смержен).
  Триаж frontier: ближайший clean-div `multi-special-ex.adoc` (87 token-diff, **single-root** — difflib: ровно 1 insert,
  9 токенов partintro-openblock, delta +9). Harness гоняет `_includes`-фрагмент книги как **standalone article** (без
  `-d book`); asciidoctor `html5.rb convert_open` ИСКЛЮЧАЕТ `[partintro]` блок (возвращает '' + error в лог), если
  `node.level > 0 || node.parent.context != :section || node.document.doctype != 'book'` — в article-режиме срабатывает
  `doctype != book`. Мы рендерили `<div class="openblock partintro">`. **Фикс (4 файла, чистый рендер-слой):**
  `lib.rs` поле `partintro_suppress: Vec<(usize,usize)>` (глубина delimited_block_stack до push + длина output до блока);
  `blocks.rs` `start_delimited_block` Open-ветка: `style==partintro && !doctype_book` → запись suppress, очистка
  `block_title_inner_html` (title тоже исключается), ничего не эмитим; `events.rs` `TagEnd::DelimitedBlock` — при совпадении
  глубины `output.truncate(pos)` + **один `\n`** (исключённый блок участвует в `blocks.join(LF)` → лидирующий join-LF →
  пустая строка на месте partintro; верифицировано пробами). Оба вида (explicit `--` И масочный `[partintro]`-параграф)
  приходят как `DelimitedBlockKind::Open` → один гейт покрывает оба. +1 html-тест `test_partintro_excluded_outside_book`
  (open-block, параграф, book-регресс). clippy `--workspace` 0; **test --workspace 0 упавших** (html 530→531, parser 645,
  compat 233, render-core 25). **Гейт 344/344 байт-в-байт** vs master `afef333` (ни один гейт-файл не ставит partintro вне
  book). **Sweep frontier(250)+adoc2docx(52)=302 new-vs-base: РОВНО 1 файл** (multi-special-ex), **0 регрессий**;
  **frontier Identical 228→229**. 5/5 CLI-проб == asciidoctor 2.0.23 (open-block+title, параграф, id+role, book-рендер,
  mid-позиция). Коммит/merge/push — ПО ЗАПРОСУ.
  - [ ] **Остаток multi-special-ex (отдельный корень, вне scope): пустая концевая секция.** `=== Appendix sub` без тела
    даёт у asciidoctor `<h3>…</h3>\n\n</div>` (шаблон секции `\n#{content}\n` при пустом content), у нас — без пустой
    строки. Token-невидимо (файл token-identical), байт-видимо. Не partintro. Дефернуто.
  - [ ] **Вырожденный edge (нереалистичный, НЕ в корпусе): partintro — ЕДИНСТВЕННЫЙ блок части** (нет глав). asciidoctor:
    `join([''])` без сепаратора → нет пустой строки; наш безусловный `\n` даёт лишнюю пустую строку. Условный `\n`
    (только при следующем сиблинге) требует lookahead/deferred-separator — не стоит риска для нереального ввода. Дефернуто.
...
- [x] **F-BD. Именованные атрибуты ссылки `id=`/`title=` на `<a>`** (ветка `fix/link-label-span-and-named-attrs`,
  ...
  **0 регрессий**. CLI-пробы (link:/bare-URL/mailto/named-only-bare) == asciidoctor. Коммит/merge/push — ПО ЗАПРОСУ.
  - [x] **F-BE. links.adoc корень A — атрибутированный span `[.role]#…#` в метке link-семейства**
    (ветка `fix/link-label-inner-span-macro`, 2026-06-23, 2-я сессия). Запрос «начни следующую задачу». master `1a3f15c`
    (F-BD смержен). Сканер метки закрывал label на ПЕРВОМ `]` (после `[.overline`); asciidoctor гонит quotes ДО macros →
    span отрендерён до поиска `]`, внутр. скобки съедены. **Фикс (4 файла):** `subst/quotes.rs` `pub(super)
    attributed_span_end(tags,src,bytes,lbrack)` — на `[` детектит атрибутированный span (`[attrlist]`+`*`/`` ` ``/`_`/`#`,
    constr+unconstr через `attrlist_constrained`/`attrlist_unconstrained`), возвращает индекс за закрывающим маркером;
    гейтинг как pass_* (unconstr — без boundary, constr — `!is_word` перед `[`); `~`/`^` не берут attrlist.
    `subst/macros.rs` `pub(super) find_link_label_close(tags,s,open)` — пропускает весь span на внутр. `[`, иначе как
    `find_macro_close_bracket` (`\]` экранируется); проводка try_link/try_mailto/try_autolink(URL[text]).
    `subst/mod.rs` `pub(crate) link_label_close(s,open)` обёртка (`&[]`, легаси без сентинелов, `TagToken` приватен).
    `inline.rs` (legacy, симметрия) 4 сайта → `link_label_close`. +1 html-тест `test_attributed_span_in_link_label_html`
    +7 кейсов parity (reproduces_legacy_on_link_inputs, pipeline==legacy). clippy `--workspace` 0; **test --workspace
    1294, 0 упавших** (html 529→530, parser 645, compat 233). **Гейт 344/344 байт-в-байт** vs master `1a3f15c`. **Sweep
    frontier(250)+adoc2docx(52): РОВНО 1 файл** (links.adoc), **0 регрессий**. **links.adoc БАЙТ-В-БАЙТ == asciidoctor
    2.0.23** (корень A+B сняты, adoc2docx Identical 44→45). CLI-пробы (autolink/link:/mailto со span, границы
    `[a [b] c]`/`[label]*next*`/leading-span) == asciidoctor. Коммит/merge/push — ПО ЗАПРОСУ.
    - [ ] **Unconstrained `[.role]##span##` в метке URL-макроса** — РАСХОЖДЕНИЕ (синтетическое, НЕ в корпусе): asciidoctor
      без boundary матчит unconstr-attrlist `[^\]]+` на самой макро-`[` (после URL — word-char), поглощая `[pre [.role]##…`
      → URL bare. Наш детект только на внутр. `[`. Constrained защищён boundary → корпус совпал. Покрытие unconstr =
      детект поглощения макро-`[` → отклонять link (др. слой, высокий риск). Дефернуто.
