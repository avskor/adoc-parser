...
## Свежий baseline корпуса (2026-06-09, ПОСЛЕ paragraph-trailing-whitespace)

`/mnt/c/tmp/adoc-test/` 344 файла: **Identical 175, Different 169, Errors 0**
...
Следующие чистые flip-кандидаты (по near-miss на 173):
...
текста терма в парсере — БОЛЬШЕ по объёму), `// end::para[]` утечка тег-региона (verse, literal),
**kbd `+`-разделитель** `kbd:[key(+key)*]` (keyboard-macro — пасстру `+...+` ест внутренний
`+`). Архитектурные (отложены): inline-monospace passthrough char-ref (`` `&#167;` `` →
Asciidoctor сохраняет в `<code>`, мы экранируем — остаток replacements.adoc; `Event::Code`),
