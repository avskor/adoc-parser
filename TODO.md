...
## Свежий baseline корпуса (2026-06-09, ПОСЛЕ verbatim-paragraph-comment)

`/mnt/c/tmp/adoc-test/` 344 файла: **Identical 184, Different 160, Errors 0**
...
Следующие чистые flip-кандидаты (по near-miss на 184):
...
текста терма в парсере — БОЛЬШЕ по объёму), **kbd `+`-разделитель** `kbd:[key(+key)*]`
(keyboard-macro — пасстру `+...+` ест внутренний `+`), **listing-blocks.adoc** (1-diff внутри
демонстрационного `------` listing с `[subs="+attributes"]` — verbatim-нюанс), **counter.adoc**
