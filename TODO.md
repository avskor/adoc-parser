...
**Рекомендация:** F-A…F-AE — ЗАКРЫТЫ (frontier identical **220**, clean-div 13). Дальше — топ clean-div (многоклассовые):
asciidoc-returns 273 (compat-backtick passthrough), sample 152 (header не распознан), manpage 146 (doctype manpage),
index 136 (table-cell literal + др.), debuter 118, multi-special-ex 87 (partintro/book), CHANGELOG 75 (xref + `\...`-escape
ellipsis), enjoy 59→227 (остаток = implicit table-header: первый ряд → `<thead>`), github-0.1.4 50 (TOC-позиция),
asciidoclet 12 (multiline footnote: `]` на следующей строке — footnote дропается, #footnotes пуст). Чистые одно-классовые
кандидаты: **implicit table-header** (enjoy — первый ряд таблицы без пустой строки после → `<thead>`); follow-up'ы F-L
(backtick-literal/single-quote в compat); F-H (`skip-front-matter`, inline `***`/`___`); `\'`-семантика (mdbasics —
asciidoctor держит `\'`, мы дропаем; + `+++`/`+-+` single-plus passthrough). Минимальные остатки (diff=1, intrinsic, не
actionable): doctime-localtime (`{localtime}`), migration (`{asciidoctor-version}`).
