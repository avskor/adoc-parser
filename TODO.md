...
- [x] **Inline/parser: callout-маркер `<N>` не прерывает top-level параграф (гейт
  ...
  **ОСТАТОК (Root 2, table.adoc флип)**: вложенная `!===`-таблица (разделитель `!`) в
  `a`-ячейке — отдельная крупная фича (протащить cell-separator `|`→`!` через
  parse_table_cells/unescape_cell_pipes/colspan; `!` в is_table_delimiter; РИСК на
  delimited.adoc где `!===` = содержимое ячейки). a-cell уже рекурсивно парсится.
