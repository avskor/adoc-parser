...
## Свежий baseline корпуса (2026-06-09, ПОСЛЕ table-header-column-style)

`/mnt/c/tmp/adoc-test/` 344 файла: **Identical 182, Different 162, Errors 0**
...
Следующие чистые flip-кандидаты (по near-miss на 182):
...
`+`), **counter.adoc** (`{counter:index}` не пишет значение в attrs → последующий `{index}`
не резолвится; п.36 — препроцессор раскрывает счётчик в локальную мапу, но плоский `{index}`
резолвит рендерер из document_attrs, куда значение не попадает).
