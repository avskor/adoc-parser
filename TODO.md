...
- [ ] **F-A. Текст ссылки/autolink режется по первой запятой** (13 файлов; самый частый). `link:url[A, B, C]` и
  `https://url[A, B]` → берём «A», asciidoctor — весь текст. Корень: `attributes.rs::parse_link_attrs` сплитит текст.
- [ ] **F-B. `:hide-uri-scheme:`** не реализован (3 файла). Атрибут срезает схему (`https://`) из ВИДИМОГО текста
  bare-autolink/`link:`: `https://github.com/foo` → текст `github.com/foo` (href неизменен). Рендерер.
- [ ] **F-G. `:source-language:`** дефолт не применяется к `[source]` без явного языка → нет `class="language-X"
  data-lang="X"` на `<code>`. Парсер/рендерер (document-attr → source-блок).
- [ ] **F-D. `toc::[attrs]`** (непустые скобки, напр. `toc::[levels=1]`) не распознаётся → параграф. `scanner::is_toc_macro`
  матчит только `toc::[]`. (Каскад: migration.adoc 861 diff = один корень.) Смежно с п.28.
- [ ] **F-C. Author / inline `<url>`** оборачивается в `mailto:` вместо bare-link/link-макроса.
  `Dan Allen <https://github.com/x>` → `mailto:https://...`; `<https://x[@t]>` → должно быть `<a href=...>@t</a>`.
- [ ] **F-F. `menu:X[]`** (один пункт, без `>`) → asciidoctor `<b class="menuref">X</b>`, мы `<span class="menu">`.
- [ ] **F-E. `[%nowrap]`** на source-блоке → класс `nowrap` не добавляется (`<pre class="highlight nowrap">`).
- [ ] **F-I. UTF-8 BOM** не срезается в начале файла → ведущий `= Title` не распознаётся как заголовок.
- [ ] **F-H. YAML front matter** (`---\n…\n---`) — мы эмитим `<hr>`; asciidoctor (без `skip-front-matter`) трактует иначе. Ниша.

**Рекомендация:** начинать с **F-A** (частый, локальный фикс парсера) → **F-G**/**F-B** (частые) → остальное.
