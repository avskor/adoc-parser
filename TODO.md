...
## Свежий baseline корпуса (2026-06-09, ПОСЛЕ link-blank-window-caret)

`/mnt/c/tmp/adoc-test/` 344 файла: **Identical 162, Different 182, Errors 0**
...
Следующие чистые flip-кандидаты (по near-miss на 162):
**bibliography-anchor `[pp]`** (1-diff кластер: `[[[pp]]]`/`<<pp>>` → reftext в скобках `[pp]`;
_crud, bibliography, subs/index, xref.adoc, _responses — `[id]` exp vs `id` got), **апостроф
`'`→’ в тексте/макросе** (scope, span-cells, README — остаток п.37), **`§`/bare char-ref**
сохранять как сущность (title-links — остаток п.15), `// end::para[]` утечка тег-региона
(verse, literal). Архитектурные (отложены): `{attr-ref}[text]` (порядок subs — icons-font,
auto-ids, custom-ids, index), link-role `class="external"` (нет поля role в `Tag::Link`).
...
- [~] **п.37 Типографские замены** (~10) — `--`→—, `...`→…, `->`→→, `'`→’ (REPLACEMENTS sub).
  ...
  - Остаток п.37: апостроф `'`→’ внутри **текста макроса** (xref/link `[...]`) не
    применяется (subs/index, span-cells, README — все `xref:...['s...]`); в plain-тексте
    апостроф уже работает. Корень — display-текст макроса не проходит REPLACEMENTS-подстановку.
...
- [x] **п.15 Entity backslash** — СДЕЛАНО (ветка `fix/escaped-char-reference`, 2026-05-31).
  ...
  **Корпус: Identical 142→145 (+3)** (multiple-authors, link-macro, ui-macros), 0 регрессий.
  Остаток п.15 (ОТДЕЛЬНАЯ бага, не backslash): **голый** `&#174;` в обычном тексте мы
  экранируем (`&amp;#174;`), а Asciidoctor СОХРАНЯЕТ валидный char-ref как сущность (`&#174;`);
  внутри listing/literal оба экранируют (совпадает). Чинить — отдельный пункт «preserve char-ref».
