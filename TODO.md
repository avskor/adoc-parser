...
**Рекомендация:** F-A/F-B/F-G/F-J/F-D/F-C/F-K/F-F/F-E/F-I/F-H/F-J'/**F-L** — ЗАКРЫТЫ. Дальше: новый frontier-проход
(топ clean-div: templates 1692, asciidoc-py 1645, migration 571, install-macos 476 — многоклассовые: attr-ref в URL,
`[x-]` спецсинтаксис, indented-section→literal); follow-up'ы F-L (backtick-literal/single-quote-emphasis в compat);
либо follow-up'ы F-H (`skip-front-matter`, inline `***`/`___`).
...
- [x] **F-V. Многострочное отступленное описание dlist схлопывалось в literalblock** (ветка
  ...
  **4 файла, ВСЕ IMPROVED, 0 регрессий** — `0-1-2-released` **432→0** (флип), `manpage` 160→146, man-asciidoctor/
  ...
  multi-term, inline-markup, nested, case C, corpus-сниппет с `{issue-ref}`). **Ожидает мержа/пуша.**
