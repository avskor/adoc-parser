...
## Свежий baseline корпуса (2026-06-09, ПОСЛЕ passthrough-inside-monospace)

`/mnt/c/tmp/adoc-test/` 344 файла: **Identical 188, Different 156, Errors 0**
...
Следующие чистые flip-кандидаты (по near-miss на 188):
**kbd `+`-разделитель** `` `+kbd:[key(+key)*]+` `` (keyboard-macro — passthrough `+...+` ест
внутренний `+`; фидли, inline passthrough-парсер), **counter.adoc** (`{counter:index}`→`{index}`
не резолвится; п.36 — АРХИТЕКТУРНЫЙ: счётчик в локальной мапе препроцессора, рендерер берёт из
document_attrs). Прежние 4-diff role.adoc + text/index.adoc ЗАКРЫТЫ (ветка
`fix/passthrough-inside-monospace`, см. ниже). Остался 4-diff **user-index.adoc**: escape макроса
внутри monospace — `` `\indexterm2:[<primary>]` `` → Asciidoctor выводит литерал `indexterm2:[<primary>]`
(`\` снимается), мы теряем `indexterm2:[`/`]` и парсим indexterm2-макрос (`<code>\&lt;primary&gt;</code>`).
Корень — escape `\` перед макросом не подавляет макрос внутри monospace-контекста (handle_inline_escape /
indexterm-парсинг). Прежнее смежное `{url}/path[text]` ЗАКРЫТО (ветка `fix/attr-ref-path-before-brackets`, см. ниже).
