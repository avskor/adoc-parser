...
**Рекомендация:** F-A…F-AF — ЗАКРЫТЫ (frontier identical **221**, clean-div 12). Дальше — топ clean-div (многоклассовые):
...
ellipsis), github-0.1.4 50 (TOC-позиция), asciidoclet 12 (multiline footnote: `]` на следующей строке — footnote дропается,
#footnotes пуст), mdbasics 3 (`\'`-escape + `+++`/`+-+` single-plus passthrough). Чистых одно-классовых кандидатов не
осталось (enjoy закрыт F-AF). Follow-up'ы: F-L (backtick-literal/single-quote в compat); F-H (`skip-front-matter`, inline
`***`/`___`); `\'`-семантика (mdbasics — asciidoctor держит `\'`, мы дропаем). Минимальные остатки (diff=1, intrinsic, не
