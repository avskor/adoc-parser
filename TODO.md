...
**Рекомендация:** F-A…F-AD — ЗАКРЫТЫ (frontier identical **220**, clean-div 13). Дальше — топ clean-div (многоклассовые):
asciidoc-returns 273 (compat-backtick passthrough), sample 152 (header не распознан), manpage 146 (doctype manpage),
index 136 (table-cell literal + др.), debuter 118, multi-special-ex 87, CHANGELOG 75, enjoy 59, github-0.1.4 50,
asciidoclet 12 (`` `data-uri` `` monospace в list-item). Чистые одно-классовые кандидаты: **callouts в markdown-fence
` ``` `** (enjoy 59, частично — есть ещё implicit table-header); follow-up'ы F-L (backtick-literal/single-quote в compat);
F-H (`skip-front-matter`, inline `***`/`___`); `\'`-семантика (mdbasics — asciidoctor держит `\'`, мы дропаем; +
`+++`/`+-+` single-plus passthrough). Минимальные остатки (diff=1, intrinsic, не actionable): doctime-localtime
(`{localtime}`), migration (`{asciidoctor-version}`).
