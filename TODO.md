...
**Рекомендация:** F-A…F-AC — ЗАКРЫТЫ (frontier identical **219**, clean-div 14). Дальше — топ clean-div (многоклассовые):
asciidoc-returns 273 (compat-backtick passthrough), sample 152 (header не распознан), manpage 146 (doctype manpage),
index 136 (table-cell literal + др.), debuter 118, multi-special-ex 87, CHANGELOG 75, enjoy 59. Чистые одно-классовые
кандидаты: **callouts в markdown-fence ` ``` `** (enjoy 59, частично — есть ещё implicit table-header); follow-up'ы
F-L (backtick-literal/single-quote в compat); F-H (`skip-front-matter`, inline `***`/`___`); `\'`-семантика (mdbasics —
asciidoctor держит `\'`, мы дропаем). Минимальные остатки (diff=1, intrinsic, не actionable): doctime-localtime
