...
- [ ] **п.41 header после комментариев** (8) — корень: `block.rs:~492` ставит
  ...
  **п.27 source-language attr** (7). АПДЕЙТ 2026-06-11: этот корень ДОМИНИРУЕТ в
  document/*-примерах корпуса (все начинаются с `// tag::…[]` перед `= Title`):
  header.adoc (69 diff), metadata (118), multiple-authors (75), title (40),
  version-label (28), part (24, + отдельно partintro-обёртка book-part) — жирный кластер.
