...
## Свежий baseline корпуса (2026-06-09, ПОСЛЕ single-plus-passthrough-constrained)

`/mnt/c/tmp/adoc-test/` 344 файла: **Identical 189, Different 155, Errors 0**
...
Следующие чистые flip-кандидаты (по near-miss на 189):
**counter.adoc** (2-diff, `{counter:index}`→`{index}` не резолвится; п.36 — АРХИТЕКТУРНЫЙ:
счётчик в локальной мапе препроцессора, рендерер берёт из document_attrs). Прежний 1-diff
keyboard-macro (`` `+kbd:[key(+key)*]+` ``) ЗАКРЫТ (ветка `fix/single-plus-passthrough-constrained`,
см. ниже). **Новый отложенный баг** (обнажён single-plus-фиксом, НЕ регрессия): trailing ` +`
...
строка 390 `` `` + +`` ``). Остался 4-diff **user-index.adoc**: escape макроса
