...
- [ ] **F-BT. `ifdef::backend-html5[]` / семейство backend-intrinsics не сеется CLI** → условные блоки для html-бэкенда
  выпадают (в cheatsheet выпадала вся passthrough-секция, хвост рассинхронизировался). Asciidoctor при html5-рендере ставит
  `backend=html5`, `backend-html5` (set), `basebackend=html`, `basebackend-html`, `filetype=html`, `filetype-html`,
  `outfilesuffix=.html`, `doctype=article`, `doctype-article`. Наш `adoc-cli` сеет docname/docdate-семейство, но НЕ backend.
  **Фикс-кандидат:** засеять backend-intrinsics в `initial_attrs`/`html_attrs` в adoc-cli/main.rs (CLI знает, что рендерит HTML).
  ⚠ Может флипнуть файлы с `ifndef::backend-html5[]` (PDF-only контент теперь корректно ДРОПается) — мерить свипом. Проверено:
  `adoc -a backend-html5 cheatsheet` даёт 2 «passthrough» = asciidoctor. Отдельная чистая задача.
