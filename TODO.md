...
- [x] **F-BI. Пустая секция: пустая строка в content-слоте (`\n#{content}\n` для пустого тела)** (ветка
  ...
  **identical**, appendix 9→1, section 18→2 (остаток = sect0); sweep frontier+adoc2docx(302) — include-with-leading-
  blank-line/multi-special-ex/sections/toc → **identical**, callouts 37→36 (остаток = Rouge). **Важный вывод:** прошлая
  «корпуса исчерпаны» опиралась на НОРМАЛИЗОВАННУЮ метрику (`frontier_parity.py` схлопывает whitespace, слепа к пустым
  строкам) — байт-паритет НЕ был исчерпан. **Новый остаток найден:** пустая строка МЕЖДУ двумя соседними
  `<h1 class="sect0">` (level-0 секции/части) — asciidoctor её даёт, мы нет (appendix/section residual); отдельный
  sect0-баг, не в scope. Коммит/merge/push — ПО ЗАПРОСУ.
