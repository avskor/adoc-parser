...
- [ ] **F-J (НОВЫЙ, обнаружен при F-G). fenced ` ``` ` → source.** asciidoctor ВСЕГДА рендерит markdown-fenced
  блоки как source (`<pre class="highlight"><code>`) даже без info-string-языка и без `source-language`; мы — plain
  listing. Не зависит от source-language (отдельный корень от F-G). `block.rs::scan_markdown_code_fence` (ветка
  без языка, ~3170) эмитит `DelimitedBlock{Listing}` — должна эмитить `SourceBlock`. Под `source-language` — с дефолтным языком.
...
**Рекомендация:** F-A/F-B/F-G — ЗАКРЫТЫ. Дальше: **F-J** (fenced→source, локальный, смежен с F-G) → **F-D**/**F-C** → остальное.
