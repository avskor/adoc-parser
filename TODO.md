...
- [x] **F-J (НОВЫЙ, обнаружен при F-G). fenced ` ``` ` → source** (ветка `feat/fenced-source`, 2026-06-18).
  ...
  - **F-J' (остаток, обнаружен при F-J): `:compat-mode:` → `:language:` алиасит `:source-language:`.** В compat-mode
    голый fence/source без явного языка берёт язык из `:language:` (вне compat-mode `:language:` НЕ влияет). Отдельный класс.
  - **F-K (предсуществующий, обнаружен при F-J): порядок классов highlightjs.** Мы эмитим `class="hljs language-X"`,
    asciidoctor — `class="language-X hljs"` (`start_source_block`, ~725). Затрагивает ВСЕ highlightjs source-блоки, не только fence.
...
**Рекомендация:** F-A/F-B/F-G/F-J — ЗАКРЫТЫ. Дальше: **F-D** (`toc::[attrs]`) / **F-C** (author/inline `<url>`) →
F-K (порядок классов highlightjs, дешёвый рендерер-фикс) → F-F/F-E/F-I/F-H/F-J'.
