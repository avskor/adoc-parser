...
## Свежий baseline корпуса (2026-06-09, ПОСЛЕ literal-unknown-style-class)

`/mnt/c/tmp/adoc-test/` 344 файла: **Identical 170, Different 174, Errors 0**
...
**kbd `+`-разделитель** `kbd:[key(+key)*]` (keyboard-macro — пасстру `+...+` ест внутренний
`+`). Архитектурные (отложены): `{attr-ref}[text]` (порядок subs — icons-font, auto-ids,
custom-ids, index), link-role `class="external"` (нет поля role в `Tag::Link`).
