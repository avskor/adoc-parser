...
## Свежий baseline корпуса (2026-06-09, ПОСЛЕ intrinsic-quot-apos-and-pass-constrained)

`/mnt/c/tmp/adoc-test/` 344 файла: **Identical 199, Different 145, Errors 0** (после
intrinsic-quot-apos-and-pass-constrained; `python3 compare_full.py`, release-бинарь). **COMPAT-DIFF.md
устарел** (числа от 2026-03-23).
Следующие near-miss-кандидаты (на 199): **pass/index** (6-diff — остаток `` `+pass:[]+` `` через
single-plus, «случай A»: асимметричный — pass извлекается ДО `+...+`, но НЕ внутри `++...++`; риск),
**stem/index** (6-diff, MathJax — архитектурный), **special-section-numbers** (10-diff, monospace в
ТЕКСТЕ xref-ссылки — архитектурный QUOTES в `[label]`), **audio** (12-diff, 2 корня). Архитектурные
кластеры (много файлов): **наследование `m`/`e`/`s` стиля колонки таблицы** (`<code>`/`<em>`/`<strong>`
в ячейках — сделано только `h`; character-replacement-ref, pass-macro, subs-group-table завязаны +
footnote `<sup>`/`stretch`-каскады), **author-header `<div class="details">`** (standalone).
