...
- [ ] **F-BU. PlantUML `@dot … @enddot` (и, вероятно, др. диаграммные) блоки** рендерятся иначе Asciidoctor (~125 diff-строк
  остатка в cheatsheet после F-BS+F-BT, район source-стр. ~303 «PlantUML Extension»). Требует отдельного showdiff-триажа.
