...
## Свежий baseline корпуса (2026-06-09, ПОСЛЕ audio-start-opts-and-title)

`/mnt/c/tmp/adoc-test/` 344 файла: **Identical 200, Different 144, Errors 0** (после
audio-start-opts-and-title; `python3 compare_full.py`, release-бинарь). **COMPAT-DIFF.md
устарел** (числа от 2026-03-23).
Следующие near-miss-кандидаты (на 200): **pass/index** (6-diff — остаток `` `+pass:[]+` `` через
...
ТЕКСТЕ xref-ссылки — архитектурный QUOTES в `[label]`). Неразведанные: **docinfo/index** (14-diff,
rowspan-ячейка размещается в конце предыдущей строки вместо начала новой — span-cell row-placement),
**db-migration** (16-diff, 2 корня: `language-source` vs `language-yaml` + пропущенный `</p>` перед
NOTE). Архитектурные кластеры (много файлов): **наследование `m`/`e`/`s` стиля колонки таблицы**
