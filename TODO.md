...
**Рекомендация:** F-A…F-AB — ЗАКРЫТЫ (frontier identical **218**, clean-div 15). Дальше — топ clean-div (многоклассовые):
asciidoc-returns 273 (compat-backtick passthrough), sample 152 (header не распознан), manpage 146 (doctype manpage),
index 136 (table-cell literal + др.), debuter 118. Чистые одно-классовые кандидаты: **callouts в markdown-fence ` ``` `**
(enjoy 59, частично — есть ещё implicit table-header); **counter/attr subs внутри literal-блока** (recommended-practices
остаток 1 — препроцессор резолвит счётчики независимо от контекста); follow-up'ы F-L (backtick-literal/single-quote
в compat); F-H (`skip-front-matter`, inline `***`/`___`); `\'`-семантика (mdbasics — asciidoctor держит `\'`, мы дропаем).
