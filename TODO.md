...
## Свежий baseline корпуса (2026-06-09, ПОСЛЕ image-alt-quotes)

`/mnt/c/tmp/adoc-test/` 344 файла: **Identical 157, Different 187, Errors 0**
...
Следующие чистые flip-кандидаты (по near-miss): xref-id норм.
`#Substitutions`→`#_substitutions` (п.19/24 — positional-and-named-attributes), alt двойная
кавычка (п.18 — author/revision-attribute-entries), `// end::para[]` утечка тег-региона (verse),
preserve bare char-ref (остаток п.15 — голый `&#174;` сохранять как сущность).
...
- [ ] **Точечные**: п.17 (остаток: `[.line-through]#`→`<del>`, `#`→`<mark>`; inline-роль
  ...
  п.19 (xref-id норм.), п.20 (`[[id,reftext]]`),
