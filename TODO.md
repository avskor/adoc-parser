...
- [x] **F-BQ. Closed-ATX заголовок секции: закрывающий run маркеров (`== Title ==`, `==== T ====`, `## MD ##`) не
  ...
  - [ ] **Свежий остаток docs-корпуса (кандидаты след. сессий):** cheatsheet (659), vue_ts_springboot ×2 (273/270),
    `_responses.adoc`/`_artifacts.adoc` (8 каждый — `=== heading` ВНУТРИ list-item: asciidoctor → параграф в элементе, у нас
    → секция; родственно read_paragraph_lines/F-BO). windows/wsl + keycloak — те же 2 архитектурных автолинка.
