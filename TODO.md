...
- [x] **F-BW. Автолинк рвался на `<`/`>` в теле URL** → URL с angle-bracket-плейсхолдерами (`http://<host>:<port>/realms/<realm>/token`)
  ...
  **keycloak 52→0** (оба файла каталога identical); docs Identical **206→207**. Коммит `f26dcd5` на ветке; merge/push — ПО ЗАПРОСУ.
  - [ ] **wsl (остаток 95, ОТДЕЛЬНЫЙ дефект):** URL `https://rubygems.org` в КОНЦЕ monospace-спана (`` `… https://rubygems.org` ``,
    URL перед закрывающим backtick без пробела) съедает закрывающий backtick (`href="…org\`"`), и сам спан НЕ оборачивается в
    `<code>`. Пре-существующий (база==new, проверено probe3), мой фикс его не задел — позиционный differ раздувает один этот
    рассинхрон в 95. Корень: `autolink_url_limit` для space-boundary внутри спана возвращает `bytes.len()` (НЕ кап на закрытие
    спана), URL мид-спана перед закрывающим маркером переедает его. Отдельная задача (constrained-span autolink cap).
