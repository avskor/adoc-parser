...
- [x] **F-BG. `replacements` в inline-link TARGET (`...`/`--`/… курлятся в href)** (ветка
  ...
  **Гейт 344/344 байт-в-байт** vs master `a8bdd1c` (0 gate-URL с `...`/`--`). **Sweep frontier(250)+adoc2docx(52)=302
  ...
  - [ ] **Остаток (вне scope, пред-существующее, BASE==NEW):** (1) `(C)`-после-буквы (`a(C)b`→`©` у asciidoctor, у нас
    литерал — лимит `apply_typographic_replacements`, есть и в плейн-тексте); (2) **mailto/email** таргеты не курлятся
